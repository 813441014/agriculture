...
function update(nodeId){
    add('编辑菜单', 'showUpdateMenu?id='+nodeId, 700, 450);
}
function callDetail(nodeId){
    detail('查看菜单', 'showUpdateMenu?id='+nodeId, 700, 450);
}
  var layout = [
...

    {
      ...
      render: function(row) {
        ...
        var str= '<a class="layui-btn layui-btn-primary layui-btn-xs" onclick="callDetail(\'' + row.id + '\')"><i class="layui-icon">&#xe615;</i> 查看</a>' +
...
function del(id,menuName) {
    layer.confirm('确定删除菜单[<label style="color: #00AA91;">' + menuName + '</label>]?', function(){
        layui.$.ajax({
            url: "del",
            type: "post",
            data: {id: id},
