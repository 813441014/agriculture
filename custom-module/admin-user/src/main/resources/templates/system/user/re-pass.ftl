...
<script>
  layui.use(['form','layer'], function(){
    ...
    form.verify({
      ...
      ,newPass: function CheckPassWord(value) {//密码必须包含数字和字母
            var str = value;
            var regNull=/\s/;
            if(regNull.exec(str)!=null){
                return "不能包含空格";
            }
            if (str == null || !/(.+){6,12}$/.test(str)) {
                return "密码必须6到12位";
            }
    ...
    form.on('submit(add)', function(data){
      $.ajax({
        ...
        success:function(json){
          ...
          window.top.layer.msg(json.msg,{icon:6,offset: 'auto',area:['120px','80px'],anim:2});
        },error:function(){
            var index = parent.layer.getFrameIndex(window.name);
          window.top.layer.msg('请求失败',{icon:5,offset: 'auto',area:['120px','80px'],anim:2});
        }
      });
      return false;
    });
